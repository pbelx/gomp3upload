...
func main() {
    ...
    router.Use(cors.New(cors.Config{
        AllowOrigins:     []string{"http://example.com"}, // Restrict to specific origins
    ...
    router.POST("/upload", func(c *gin.Context) {
        ...
        if file.Size > 10*1024*1024 {
            ...
            c.String(http.StatusBadRequest, "file size exceeds 10MB limit")
            return
        }
        ...
        log.Printf("file uploaded successfully: %s", savePath)
    ...
    router.GET("/files", func(c *gin.Context) {
        ...
        if err != nil {
            log.Printf("could not list files: %s", err.Error())
